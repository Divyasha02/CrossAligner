...
from .util import (
    build_outputdict,
    threshold_finder,
    confidence_score_ratio_based_filtering,
    confidence_score_based_filtering,
...
def retriever_postprocessor_SVM(predicts: List, threshold: float = 0.0) -> List:
    predicts_temp = []
    for predict in predicts:
        source = predict["source"]
        targets = predict["target-cands"]
        scores = predict["score-cands-norm"]   # USE NORMALIZED SCORES

        for t, s in zip(targets, scores):
            if s >= threshold:                # threshold works now
...
def llm_relation_postprocessor(predicts: List, dataset: Any, relation_key: str = "relation") -> List:
    """
    Post-processes descriptive LLM outputs by extracting a JSON object per prediction and
    returning normalized relation outputs.
    ...
    if len(predicts) != len(dataset_items):
        raise ValueError(
            f"Length mismatch: got {len(predicts)} predictions but {len(dataset_items)} dataset items. "
            "Ensure prediction order matches dataset order and that you decoded one text output per input prompt."
        )

    parsed_outputs = []
    for predict, data in zip(predicts, dataset_items):
        cleaned = truncate_generation_noise(predict)
        parsed = extract_first_json_object(cleaned) or extract_first_json_object(predict)
        if parsed is None and isinstance(predict, str):
            relation_match = re.search(
                r"(?:\"relation\"|relation)\s*[:=]\s*\"?([a-zA-Z_ -]+)\"?",
                predict,
                flags=re.IGNORECASE,
            )
            ...
            if relation_match:
                parsed = {
                    relation_key: relation_match.group(1),
                    "reason": reason_match.group(1).strip() if reason_match else "heuristic_parse",
                }
            else:
                simple_relation = normalize_relation_label(predict.strip().splitlines()[0])
                if simple_relation != "uncertain":
                    parsed = {
                        relation_key: simple_relation,
                        "reason": "heuristic_parse_relation_only",
                    }
        if parsed is None:
        ...
        relation = normalize_relation_label(parsed.get(relation_key, "uncertain"))
...
def llm_relation_postprocessor(predicts: List, dataset: Any, relation_key: str = "relation") -> List:
    ...
    for predict, data in zip(predicts, dataset):
        ...
        relation = normalize_relation_label(parsed.get(relation_key, "no_relation"))
        reason = parsed.get("reason", "")
        parsed_outputs.append(
            {
                "source": data["iris"][0],
