...
class LLM(BaseOMModel):
    ...
    def tokenize(self, input_data: List) -> Any:
        ...
        if (
            self.kwargs.get("use_chat_template", True)
        ...
        dispatch_map = getattr(self.model, "hf_device_map", None)
        if dispatch_map:
            dispatch_devices = []
            for mapped_device in dispatch_map.values():
                if isinstance(mapped_device, int):
                    dispatch_devices.append(f"cuda:{mapped_device}")
                elif isinstance(mapped_device, str):
                    dispatch_devices.append(mapped_device)

            preferred_device = next(
                (
                    device_name for device_name in dispatch_devices
                    if device_name not in {"cpu", "disk", "meta"}
                ),
                "cpu",
            )
            inputs.to(preferred_device)
        else:
            inputs.to(self.kwargs["device"])
        return inputs
...
class BaseLLMArch(LLM):
    ...
    def load_model(self, path: str) -> None:
        ...
        if self.kwargs["device"] != "cpu":
            ...
            self.model = self.model.from_pretrained(path, device_map=self.kwargs.get("device_map", "auto"))
        else:
...
class DecoderLLMArch(BaseLLMArch):
    ...
    def load_model(self, path: str) -> None:
        ...
        if llm_req_hugging_tk:
            model_kwargs["token"] = self.kwargs["huggingface_access_token"]
        ...
        if self.kwargs["device"] != "cpu":
            ...
            device_map = self.kwargs.get("device_map", "balanced")
            if (
                device_map == "auto"
                and isinstance(quantization_config, BitsAndBytesConfig)
                and getattr(quantization_config, "load_in_8bit", False)
            ):
                device_map = "balanced"
            model_kwargs["device_map"] = device_map
            model_kwargs["low_cpu_mem_usage"] = self.kwargs.get("low_cpu_mem_usage", True)
            self.model = self.model.from_pretrained(path, **model_kwargs)
            return
