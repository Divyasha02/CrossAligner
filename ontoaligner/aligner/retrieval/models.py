...
class GTERetrieval(BiEncoderRetrieval):
    def __init__(self, model_name: str = "Alibaba-NLP/gte-large-en-v1.5", batch_size: int = 32, **kwargs):
        super().__init__(**kwargs)
        self.model_name = model_name
        self.batch_size = batch_size

    def load(self, path: str = None):
        path = path or self.model_name
        self.model = SentenceTransformer(path, device=self.kwargs["device"], trust_remote_code=True)

    def fit(self, inputs):
        return self.model.encode(inputs, show_progress_bar=True, batch_size=self.batch_size)

    def transform(self, inputs):
        return self.model.encode(inputs, show_progress_bar=True, batch_size=self.batch_size)

    def __str__(self):
        return super().__str__() + "+GTERetrieval"
